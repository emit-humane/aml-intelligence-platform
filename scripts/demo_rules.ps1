...
# transaction is ingested).  So a rule that fires when count >= N needs
# N prior transactions already in the store — the (N+1)th tx is the trigger.
#
...
Write-Host "`n  Run ID: $RUN  (suffix on all IDs — fresh state every run)" -ForegroundColor DarkGray
...
function Post-Tx($body, [string]$label="") {
    try {
        ...
        Write-Host ("  {0,-26} score={1,6}  level={2,-8}  rules=[{3}]" `
...
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R02] Structuring  (need 3 prior sub-threshold txns in 1 h; fires on 4th)" -ForegroundColor Magenta
...
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R03] Velocity Spike  (need 11 prior txns in 1 h; fires on 12th)" -ForegroundColor Magenta

for ($j = 0; $j -lt 12; $j++) {
    $min   = "{0:D2}" -f ($j * 4)          # 0,4,8,...,44  all within 1 hour
    $txid  = "R03_{0:D3}" -f ($j + 1)
    $rxacc = "R03_RX{0:D2}" -f ($j + 1)
    $label = if ($j -eq 11) { "R03_TRIGGER(pre-v=11)" } else { "R03_seed$($j+1)(pre-v=$j)" }
...
#      Step 1: seed with a 2023 timestamp to set last_tx_ts_ever
...
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R06] High-Risk Jurisdiction  (IN -> PK, is_international=True)" -ForegroundColor Magenta
...
#      First-ever tx from a fresh account with a real device_id
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R07] Device Anomaly  (new device + amount > 200K)" -ForegroundColor Magenta

$devR07 = "DEV_R07_${RUN}"   # per-run device keeps it fresh
...
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R08] Shared Device  (need 6 prior accounts on same device; fires on 7th)" -ForegroundColor Magenta
...
1..7 | ForEach-Object {
    ...
    $label = if ($i -eq 7) { "R08_TRIGGER(pre-cnt=6)" } else { "R08_seed$i(pre-cnt=$($i-1))" }
...
#      already stored before the 22nd evaluation reaches count=21 > 20
#      → send 22 txns to 22 unique receivers; rule fires on the 22nd
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R09] Excessive Beneficiaries  (need 21 prior unique receivers; fires on 22nd)" -ForegroundColor Magenta

1..22 | ForEach-Object {
    $i    = $_
    $txid = "R09_{0:D3}" -f $i
    $recv = "R09_RECV{0:D2}" -f $i
    # [int] cast required: [math]::Floor returns double, {0:D2} requires integer
    $day  = "{0:D2}" -f [int]([math]::Floor(($i - 1) / 4) + 1)
    $hr   = "{0:D2}" -f [int]((($i - 1) % 4) * 6)
    $label = if ($i -eq 22) { "R09_TRIGGER(pre-cnt=21)" } else { "R09_seed$i(pre-cnt=$($i-1))" }
...
#      Build A→B→C→A; graph checks if adding edge (src,dst) would
#      close a cycle → C→A is the first edge that closes one
# ──────────────────────────────────────────────────────────────────
...
Post-Tx (Tx -id "R10_001" -sender "R10_A" -receiver "R10_B" `
            -amount 500000 -ts "2024-07-04T14:00:00Z") "R10_A->B (no cycle)"
Post-Tx (Tx -id "R10_002" -sender "R10_B" -receiver "R10_C" `
            -amount 490000 -ts "2024-07-04T14:30:00Z") "R10_B->C (no cycle)"
...
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R11] Round Amount Structuring  (need 6 prior round txns in 24 h; fires on 7th)" -ForegroundColor Magenta

$r11amounts = @(300000, 500000, 200000, 100000, 400000, 600000, 700000)
for ($j = 0; $j -lt 7; $j++) {
    $hr    = "{0:D2}" -f ($j * 3)          # 00,03,06,09,12,15,18 — all within 24 h
    $txid  = "R11_{0:D3}" -f ($j + 1)
    $rxacc = "R11_RX{0:D2}" -f ($j + 1)
    $label = if ($j -eq 6) { "R11_TRIGGER(pre-v=6)" } else { "R11_seed$($j+1)(pre-v=$j)" }
...
#      → fires from the 6th transaction onward (5 prior amounts)
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R13] Benford Anomaly  (all amounts start with 9; fires from tx6)" -ForegroundColor Magenta

$r13amounts = @(90000,91000,92000,93000,94000,95000,96000,97000,98000,99000,
                90500,91500,92500,93500,94500)
for ($j = 0; $j -lt 15; $j++) {
    $hr    = "{0:D2}" -f ($j + 1)          # 01..15  all within same day
    $txid  = "R13_{0:D3}" -f ($j + 1)
    $rxacc = "R13_RX{0:D2}" -f ($j + 1)
    # chi2 requires >= 5 prior amounts in all_amounts (pre-event)
    # tx1-tx5: 0-4 prior → chi2=0; tx6+: 5+ prior → chi2 fires
    $label = if ($j -ge 5) { "R13_TRIGGER_tx$($j+1)(5+prior)" } else { "R13_seed$($j+1)(${j}prior)" }
...
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R14] Fan-In Collector  (need 3 prior unique senders; fires on 4th)" -ForegroundColor Magenta

1..4 | ForEach-Object {
    $i     = $_
    $txid  = "R14_{0:D3}" -f $i
    $src   = "R14_SRC{0:D2}" -f $i
    $label = if ($i -eq 4) { "R14_TRIGGER(pre-indeg=3)" } else { "R14_seed$i(pre-indeg=$($i-1))" }
...
#      Step 1: seed R15_MID as a receiver (in_degree becomes 1)
#      Step 2: R15_MID sends >= 500K forward → sender_in_degree=1 → fires
# ──────────────────────────────────────────────────────────────────
Write-Host "`n[R15] Pass-Through Layering  (receive first, then forward >= 500K)" -ForegroundColor Magenta
...
Write-Host "  Rule  | Fires on      | Condition (PRE-event count)" -ForegroundColor Yellow
Write-Host "  ------+---------------+-----------------------------" -ForegroundColor Yellow
Write-Host "  R01   | tx1           | amount > 1,000,000" -ForegroundColor Yellow
Write-Host "  R02   | tx4           | v1h >= 3  (3 prior sub-threshold txns)" -ForegroundColor Yellow
Write-Host "  R03   | tx12          | v1h > 10  (11 prior txns in 1h)" -ForegroundColor Yellow
Write-Host "  R04   | tx2           | gap_seconds > 15.5M  (180 days)" -ForegroundColor Yellow
...
Write-Host "  R11   | tx7           | v24h > 5  (6 prior round-amount txns)" -ForegroundColor Yellow
